...
**Autograd (Low-level):**
`torch.autograd.grad`, `torch.autograd.functional.jacobian()` (compute the Jacobian matrix of a function with respect to its inputs, e.g. `jacobian(fn, inputs)` for Jacobian-lens steering and coordinate patching)
