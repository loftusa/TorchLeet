...
**Python Builtins:**
`isinstance()`, `hasattr()`, `setattr()`, `max(iterable, key=...)`, `max(iterable, key=dict_obj.get)` (using `dict.get` as a key callable, e.g. in BPE merge scoring), `min(iterable, key=...)`, `sorted(iterable, key=..., reverse=True)`, `property` (decorator), `classmethod` (decorator), `str.replace(old, new)`, `str.split()`, `str.join(iterable)` (e.g. `' '.join(tokens)`), `str.lower()`, `str.strip()`, `str.isalpha()`, `str.islower()`, `str.isupper()`, `str.isdigit()`, `str.startswith()`, `tuple(iterable)`, `set()`, `next()`, `enumerate()`, `zip()`, `float('nan')` (explicit NaN sentinel value), `float("-inf")` (string-form negative infinity, used as attention mask fill value)
