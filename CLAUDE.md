...
**tkinter:**
`tk.Tk()`, `tk.Button`, `tk.Label`, `tk.Frame`, `tk.Canvas()`, `tk.StringVar()` (tkinter observable string variable, used to bind widget text to a Python variable), `root.mainloop()`, `widget.config()`, `widget.grid()`, `widget.pack()`, `widget.bind()` (event binding)
