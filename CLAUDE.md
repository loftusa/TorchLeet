...
**NumPy:**
`np.ascontiguousarray`, `np.transpose`, `np.stack`, `np.argsort`, `np.argmax`, `np.argpartition`, `np.partition`, `np.zeros`, `np.array`, `np.mean`, `np.std`, `np.min`, `np.max`, `np.sum`, `np.abs`, `np.percentile`, `np.exp`, `np.log`, `np.log2`, `np.log10`, `np.average`, `np.linspace`, `np.concatenate`, `np.column_stack`, `np.clip`, `np.sort`, `np.meshgrid`, `np.random.seed`, `np.random.choice`, `np.random.randint`, `np.random.normal`, `np.random.RandomState(seed)`, `np.linalg.eigh`, `np.linalg.norm()`, `np.trace`, `np.repeat`, `np.tile`, `np.arange`, `np.empty`, `np.random.permutation`, `np.append()`, `np.pi` (π constant)
