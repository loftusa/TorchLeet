...
10. [Role Confusion Probes](papers/role-confusion-probes/role-probes-Question.ipynb) [(Solution)](papers/role-confusion-probes/_solutions/role-probes.ipynb) - Linear role probes on a base model's hidden states: train on tags with content held constant, then show CoT-styled text in `<user>` tags still reads as ~0.6 CoTness — style hijacks tags, the mechanism behind prompt injection ([Ye et al., ICML 2026](https://arxiv.org/abs/2603.12277))
11. [Probing Misaligned Thinking](papers/probe-misalignment/probe-misalignment-Question.ipynb) [(Solution)](papers/probe-misalignment/_solutions/probe-misalignment.ipynb) - Per-indicator linear probes on residual activations, OR-fusion monitor, threshold tuning at a fixed neutral FPR, and transcript-TPR/AUROC evaluation ([Zhou et al., 2026](https://arxiv.org/abs/2606.24251))
12. [Jacobian Lens](papers/jacobian-lens/jacobian-lens-Question.ipynb) [(Solution)](papers/jacobian-lens/_solutions/jacobian-lens.ipynb) - Averaged Jacobian to the final layer as a principled logit-lens replacement: exact causal-block Jacobians via autograd, lens reads, J-lens steering (reading direction = gradient), pseudoinverse coordinate patching, and non-negative matching pursuit for the verbalizable "J-space" fraction ([Transformer Circuits, 2026](https://transformer-circuits.pub/2026/workspace/index.html))
13. [TDKPS Perspective Shifts](papers/tdkps-perspective-shifts/tdkps-Question.ipynb) [(Solution)](papers/tdkps-perspective-shifts/_solutions/tdkps.ipynb) - Monitoring behavioral drift in black-box multi-agent systems: omnibus distance matrix over (agent, time) snapshots, classical MDS (Torgerson double-centering), agent-level drift scores, and a group-level energy-distance paired-permutation test ([Bridgeford & Helm, 2025](https://arxiv.org/abs/2512.05013))
...
6. [Control Charts for Multi-Agent Systems](practice/06-Control-Charts-Multi-Agent/control-charts-Question.ipynb) [(Solution)](practice/06-Control-Charts-Multi-Agent/_solutions/control-charts.ipynb) - Statistical process control for agent deployments ([Helm, Priebe & Duderstadt, 2026](https://arxiv.org/abs/2605.11135)): iso-mirror trajectory via classical MDS, adaptive Shewhart chart with rolling control limits, sigmoid slow-defection sleeper agent, and empirical reproduction of both headline results — slow defection evades adaptive monitoring (Theorem 1), and no chart is both adaptive and secure (Theorem 2)
