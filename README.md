...
12. [DSpark](papers/dspark/dspark-Question.ipynb) [(Solution)](papers/dspark/_solutions/dspark.ipynb) - Confidence-scheduled speculative decoding: a semi-autoregressive Markov draft head (low-rank transition bias added to a parallel backbone), the acceptance identity `c* = 1 - ½‖p_d-p_t‖₁ = Σ min(p_d,p_t)`, and the hardware-aware prefix scheduler (greedy throughput maximization by cumulative survival, validated against a brute-force oracle) (DeepSeek-AI & Peking University, [DeepSpec](https://github.com/deepseek-ai/DeepSpec))
